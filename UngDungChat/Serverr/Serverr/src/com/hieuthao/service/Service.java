...
public class Service {
        ...
        server.addEventListener("send_file", Model_Package_Sender.class, new DataListener<Model_Package_Sender>() {
            ...
            public void onData(SocketIOClient sioc, Model_Package_Sender t, AckRequest ar) throws Exception {
                try {
                    ...
                    if (t.isFinish()) {
                        ar.sendAckData(true);
                        Model_Reicever_Img dataImage = new Model_Reicever_Img();
        ...
        if (data.getMessageType() == MessageType.IMAGE.getValue() || data.getMessageType() == MessageType.FILE.getValue()) {
            ...
            for (Model_Client c : listClient) {
                if (c.getUser().getUserID() == data.getToUserID()) {
                    c.getClient().sendEvent("receive_ms", new Model_Receive_Message1(data.getMessageType(), data.getFromUserID(), data.getText(), null));
                    break;
                }
            }
        }
    ...
    private void sendTempFileToClient(Model_Send_Message data, Model_Reicever_Img dataImage) {
        for (Model_Client c : listClient) {
            if (c.getUser().getUserID() == data.getToUserID()) {
                c.getClient().sendEvent("receive_ms", new Model_Receive_Message1(data.getMessageType(), data.getFromUserID(), data.getText(), dataImage));
                break;
            }
        }
    }
